...
class LootBox:
    ...
    def __get_preferences(self, uuid, dbc):
        """Collects the users preferences from a database, based on their UUID"""
        result = dbc.get_preferences(uuid)
        return result

    def __collect(self, preferences, dbc):
        """Collects all articles of clothing based on the preferences of the user prefs"""
        result = []
    ...
    def __set_probabilities(self, collected_items, dbc):
        """Once items has been collected, they need to have assigned random probabilities between their min &
        max """
        result = []
    ...
    def __draw_items(self, probabilitiy_assigned_items, lootbox_type):
        """Draw items based on probabilities"""        
